...
#import "MNSideMenuViewController.h"

@interface MNSideMenuViewController ()<UIGestureRecognizerDelegate>
...
@implementation MNSideMenuViewController{
...
#warning 背景图片
    [g_backGroundImage setImage:[UIImage imageNamed:@"home_bg.jpg"]];
...
-(void)setNewRootViewController:(UIViewController *)viewController animation:(BOOL)animation{
    ...
    if (g_menuFlags.showingLeftView) {
        ...
        CGAffineTransform transScale = CGAffineTransformMakeScale(_scaleSize, _scaleSize);
        CGAffineTransform transConcat = CGAffineTransformConcat(transRight, transScale);
        __block MNSideMenuViewController *selfRef = self;
...
- (void)showShadow:(BOOL)val {
    ...
    _rootViewController.view.layer.shadowOpacity = val ? 0.5f : 0.0f;
    if (val) {
